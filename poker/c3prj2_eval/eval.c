...
int compare_hands(deck_t * hand1, deck_t * hand2) {
  ...
  hand_eval_t ht = evaluate_hand(hand1);
  hand_eval_t htt = evaluate_hand(hand2);

  if(ht.ranking != htt.ranking){
    return htt.ranking - ht.ranking;
  }

  for(int i = 0; i < 5; ++i){
    if(ht.cards[i]->value == htt.cards[i]->value){
      continue;
    }else{
      return ht.cards[i]->value - htt.cards[i]->value;
    }
  }
  return 0;
}
